...
class IndependentSet:
    ...
    def __init__(self, num_vertex, edges):
    ...
    def evaluate(self, state):
        ...
        #     if (set(edge_list[i]).intersection(state)

        # for i in range(len(vertices)):
        #     for j in range(i + 1, len(vertices)):
        #         if (state[i] == 1 && state[j] == 1 && edge_list[i].contains(j)):
        #             break
...
class StringMatch:
    ...
    def evaluate(self, state):
        ...
        return np.count_nonzero(state!=self.match)
...
class TwoEnds():
    ...
    def evaluate(self, state):
        ...
        fitness = max(oneCount,zeroCount)
        # if (oneCount > self.T and zeroCount > self.T):
        #     fitness = oneCount * zeroCount
        # else:
        #     fitness = np.max([oneCount, zeroCount])
...
class RandomMax:
    ...
    def __init__(self, length, maxItem=None):
        self.prob_type = 'discrete'
        self.weights = np.random.randint(0, length * 2, length)
        if maxItem is None:
            self.maxItem = length / 4
        else:
            self.maxItem = maxItem

    def evaluate(self, state):

        if(np.sum(state) >  self.maxItem):
            return 0

        fitness = np.sum(state * self.weights)
        return fitness
...
class PatternFind:
    ...
    def evaluate(self, state):
        ...
        count = 0
        for i in range(len(state) - len(self.pattern)):
            if (state[i : i + len(self.pattern)] == self.pattern).all():
                count += 1
...
class ParityHalfString:
    ...
    def evaluate(self, state):
        ...
        parity = np.count_nonzero(state) % 2
        palindromeCount = 0
        # print(state)
        for i in range(len(state) // 2):
            if ( state[i] == state[i + len(state) // 2]):
                palindromeCount += 1
        fitness = parity * palindromeCount
